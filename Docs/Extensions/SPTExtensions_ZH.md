...
2. 安装本模组与SuntionCore模组到SPT(本模组Releases默认打包为适配通用模组安装方式的格式), 把`你的游戏文件夹\SPT\user\mods\SuntionCore\SuntionCore.dll"`和`"你的游戏文件夹\SPT\user\mods\SuntionCore.SPTExtensions\SuntionCore.SPTExtensions.dll"`复制到上述libs文件夹下
3. 在`你的项目名称.csproj`中写入以下内容
   ```css
    <ItemGroup>
        ...
        <Reference Include="SuntionCore.SPTExtensions.dll">
